...
chartypes = yaml.load(open("SCAchars.yaml", "r"), Loader=yaml.FullLoader)
...
class SCARule:
    ...
    def __init__(self, sca, f, n, s, last, verbose = False):
        ...
        self.rexp = "".join(["(%s)" % P.getExtRexp() for P in (self.pre, self.before, self.post)])
        if verbose:
...
#            part.showDetailed("-----------")
    ...
    def apply(self, d, word, E = None, verbose = False):
        ...
        while True:
            ...
            # print(self.after, self.after.getText())
            if m is not None:
